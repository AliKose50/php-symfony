...
function initPage() {
    ...
    const hasPlaceholders = document.querySelector(
        "#navbar-placeholder, #hero-placeholder, #content-placeholder, #cart-placeholder, #footer-placeholder"
    );
    if (hasPlaceholders) {
        loadComponents();
    } else {
    ...
    try {
        ...
        const promises = components.map(async (comp) => {
            ...
            if (element) {
                ...
                if (!response.ok)
                    throw new Error(`Dosya yüklenemedi: ${comp.url}`);
        ...
        .then((data) => {
            ...
            if (data.success) {
                ...
                const qtySpan = document.querySelector(
                    "#cart-item-" + itemId + " .font-medium.px-3"
                );
                if (qtySpan) qtySpan.textContent = data.quantity;

                // Update line total
                const lineTotalEl = document.getElementById(
                    "item-total-" + itemId
                );
                if (lineTotalEl)
                    lineTotalEl.textContent =
                        (data.lineTotal || 0).toFixed(2).replace(".", ",") +
                        " ₺";
                ...
                if (subtotalEl)
                    subtotalEl.textContent =
                        (data.cartTotal || 0).toFixed(2).replace(".", ",") +
                        " ₺";
                if (totalEl)
                    totalEl.textContent =
                        (data.cartTotal || 0).toFixed(2).replace(".", ",") +
            ...
            .then((data) => {
                ...
                if (data.success) {
                    ...
                    if (remaining === 0) {
                        ...
                        if (mainEl) {
                            mainEl.innerHTML =
                                '<div class="p-8 bg-white rounded-lg text-center"><p class="text-gray-700">Sepetinizde ürün bulunmuyor.</p><a href="/product" class="mt-4 inline-block bg-primary-600 text-white px-4 py-2 rounded">Alışverişe Başla</a></div>';
                        }
                    }
                } else {
                    alert("Hata: " + (data.error || "Bilinmeyen hata"));
    ...
    fetch("/cart/add/" + productId, {
        ...
        .then((response) => {
            ...
            if (!ct.includes("application/json")) {
                ...
                return response.text().then((txt) => {
                    ...
                    alert(
                        "Sunucudan beklenmeyen bir cevap alındı. Detaylar konsolda."
                    );
                    return null;
                });
            }
        ...
        .then((data) => {
            ...
            if (typeof data.cartCount !== "undefined") {
                ...
                const prev = c0
                    ? parseInt((c0.textContent || "").replace(/[^0-9]/g, "")) ||
                      0
                    : 0;
                newCount = prev + 1;
            }
            ...
            try {
                const addBtn =
                    ...
                    ) || document.querySelector(`#add-to-cart-${productId}`);
                if (addBtn) {
        ...
        .catch((error) => {
            ...
            alert(
                "Sepete ekleme işlemi sırasında hata oluştu. Konsolu kontrol edin."
            );
        });
};
        ...
        .then((data) => {
            ...
            if (btn) {
                if (data.isFavorite) {
                    ...
                    btn.className =
                        "flex-1 bg-gray-600 text-white font-semibold py-3 px-4 rounded-lg cursor-not-allowed";
                    btn.disabled = true;
                } else {
                    btn.textContent = "❤️ Favoriye Ekle";
                    btn.className =
                        "flex-1 bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-4 rounded-lg transition-colors duration-200 favorite-btn";
                    btn.disabled = false;
                }
            }
        })
...
function showTempMessage(msg) {
    try {
        ...
        el.textContent = msg;
        el.style =
            "position:fixed; top:1rem; right:1rem; background:#16a34a; color:white; padding:8px 12px; border-radius:6px; box-shadow:0 6px 18px rgba(0,0,0,0.12); z-index:9999; font-family:Inter, sans-serif;";
